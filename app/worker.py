...
Each job uses a targeted MongoDB $set update so the AI job and plagiarism job
can run concurrently without clobbering each other's fields.
"""
import asyncio
...
from app.services.groq_service import detect_ai_writing_full, analyze_plagiarism
...
async def startup(ctx: dict) -> None:
    """Open a dedicated Motor/Beanie connection for this worker process."""
    client = AsyncIOMotorClient(settings.MONGODB_URI)
    await init_beanie(
        database=client["scanvault"],
        document_models=[User, Plan, Payment, ScanDocument],
    )
    ctx["motor_client"] = client


async def shutdown(ctx: dict) -> None:
    ctx["motor_client"].close()


# ── Job: AI Detection ───────────────────────────────────────────────────────


async def analyze_ai_job(ctx: dict, doc_id: str) -> None:
    """
...
# ── Job: Plagiarism Detection ───────────────────────────────────────────────


async def analyze_plagiarism_job(ctx: dict, doc_id: str) -> None:
    """
...
class WorkerSettings:
    functions = [analyze_ai_job, analyze_plagiarism_job]
    on_startup = startup
    on_shutdown = shutdown
    redis_settings = RedisSettings.from_dsn(settings.REDIS_URL)
    max_jobs = 10
    job_timeout = 600  # seconds — 10-minute ceiling per job
